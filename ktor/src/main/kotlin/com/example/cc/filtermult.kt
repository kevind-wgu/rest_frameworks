...
fun main() {
    ...
    val output = calc(2020, input)
    println("OUTPUT: $output")
    if ( output == 514579 ) println("HURRAY") else throw RuntimeException("BAD")
}

fun calc(match: Int, input: List<Int>): Int {
    var ret = 0;
    for ( aIndex in input.indices ) {
        val aVal = input.get(aIndex)
        for ( bIndex in (aIndex+1)..input.size-1) {
            val bVal = input.get(bIndex)
            val result = aVal + bVal
            println("$aVal:$bVal = $result  ? $match")
            if (result == match) {
                ret = aVal * bVal
                break;
            }
